...
N = length(t);
xd = zeros(N,1); yd = zeros(N,1); thetad = zeros(N,1);
e1 = zeros(N,1); e2 = zeros(N,1); e3 = zeros(N,1);
v_cmd = zeros(N,1); w_cmd = zeros(N,1);

for i=1:N
    [~, ref, err, ctrl] = unicycle_dynamics(t(i), q(i,:)');
    xd(i) = ref(1); yd(i) = ref(2); thetad(i) = ref(3);
    e1(i) = err(1); e2(i) = err(2); e3(i) = err(3);
    v_cmd(i) = ctrl(1); w_cmd(i) = ctrl(2);
end
